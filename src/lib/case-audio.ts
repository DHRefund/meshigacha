...
export class CaseAudio {
  ...
  preload() {
    ...
    for (const name of names)
      void (this.context ? this.decode(name) : this.fetchSound(name)).catch(
        () => {},
      );
  }
  ...
  private fetchSound(name: CaseSound) {
    ...
    if (!request) {
      request = fetch(`${this.base}/sounds/${name}.mp3`).then((r) => {
  ...
  unlock() {
    ...
    try {
      ...
      /* Audio availability never blocks the roulette. Next gesture retries. */
    }
  }
  play(name: CaseSound) {
  ...
  setMuted(muted: boolean) {
    this.muted = muted;
    if (this.gain) this.gain.gain.value = muted ? 0 : 0.65;
    if (muted) this.pause();
    else this.unlock();
  }
  pause() {
