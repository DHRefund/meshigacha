...
export const emptyProfile = (): PoolProfile => ({
  disabled: [],
  custom: [],
  revision: 0,
});
...
export function validateProfile(input: unknown): PoolProfile {
  ...
  if (
    Object.keys(p).some(
      (k) => !["disabled", "custom", "revision"].includes(k),
    ) ||
  ...
  if (
    p.disabled.length > foods.length ||
    p.custom.length > 50 ||
    new Set(p.disabled).size !== p.disabled.length ||
    p.disabled.some((id) => !ids.has(id))
  )
    throw new Error("Invalid dishes");
  const custom = p.custom.map((item: unknown): CustomFood => {
    if (!item || typeof item !== "object") throw new Error("Invalid dish");
    const f = item as Record<string, unknown>;
    if (
      Object.keys(f).some((k) => !["id", "name", "price", "veg"].includes(k)) ||
      typeof f.id !== "string" ||
      ...
      (f.price as number) < 10 ||
      (f.price as number) > 500 ||
      typeof f.veg !== "boolean"
    )
      throw new Error("Invalid dish");
    return {
      id: f.id,
      name: f.name.trim().normalize("NFC"),
      price: f.price as number,
      veg: f.veg,
    };
  });
  if (
    new Set(custom.map((f) => f.id)).size !== custom.length ||
    foods.length - p.disabled.length + custom.length < 1
  )
    throw new Error("Keep at least one dish");
  return {
    disabled: p.disabled as number[],
    custom,
...
export function personalSelector(population: Food[], target: number) {
  ...
  const feasible = Math.max(
    Math.min(...population.map((f) => f.price)),
    Math.min(target, Math.max(...population.map((f) => f.price))),
  );
