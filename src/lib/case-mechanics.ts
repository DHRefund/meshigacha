...
export const TARGET_LUNCH_PRICE = 800;
...
export function createFoodSelector<T extends PricedMeal>(
  ...
  // VIP Boost Multiplier: Boost drop rate of Deluxe (Red) & VIP (Gold) dishes so users get rewarded with awesome food!
  // Rarity 0: 1.0x | Rarity 1: 1.1x | Rarity 2: 1.25x | Rarity 3 (Red): 1.45x | Rarity 4 (Gold VIP): 1.65x
  const vipBoostMultipliers = [1.0, 1.1, 1.25, 1.45, 1.65];
  const logs = population.map((f) => Math.log(f.price / 800));
  const prior = logs.map(
    (x, i) =>
      -0.5 * (x / LOG_PRICE_SPREAD) ** 2 -
      Math.log(counts.get(population[i].price)!) +
      Math.log(vipBoostMultipliers[population[i].rarity] ?? 1.0),
  );
...
export function priceRarity(
  priceInYen: number,
  category: 'main' | 'drinks' | 'snacks' | 'pub' = 'main',
) {
  if (category === 'drinks') {
    return priceInYen <= 300
      ? 0
      : priceInYen <= 400
        ? 1
        : priceInYen <= 500
  ...
  // Default 'main' foods category
  return priceInYen <= 600
    ? 0
    : priceInYen <= 950
      ? 1
      : priceInYen <= 1300
