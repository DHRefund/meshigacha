        ...
        {variant === "inventory" ? (
          <>
            ...
            <span className="customize-full">
              {ja ? "メニューカスタマイズ" : vi ? "Tuỳ chỉnh món ăn" : "Customize food"}
            </span>
            <span className="customize-short">
              {ja ? "設定" : vi ? "Tuỳ chỉnh" : "Customize"}
            </span>
          </>
        ) : (
          <>
            <SlidersHorizontal size={17} />
            <span>{ja ? "マイメニュー" : vi ? "Món của tôi" : "My dishes"}</span>
          </>
        )}
      </button>
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="preferences-dialog">
          <DialogTitle>{ja ? "マイメニュー設定" : vi ? "Món của tôi" : "My dishes"}</DialogTitle>
          <DialogDescription>
            {ja
              ? "選択内容は端末のクッキーに自動保存されます。"
              : vi
          ...
          <div className="pool-tabs">
            ...
            >
              {ja ? "標準メニュー" : vi ? "Món có sẵn" : "Catalog"} (
            ...
            >
              {ja ? "カスタム料理" : vi ? "Món tự thêm" : "Custom"} ({draft.custom.length}/50)
            </button>
          </div>
              ...
              {tab === "builtIn" ? (
                <>
                  <input
                    ...
                    placeholder={ja ? "料理を検索…" : vi ? "Tìm món…" : "Search dishes…"}
                    aria-label={ja ? "料理を検索" : vi ? "Tìm món" : "Search dishes"}
                    value={search}
                ...
                <>
                  <div className="custom-form">
                    ...
                    <label>
                      {ja ? "目安価格 (千VND)" : vi ? "Giá (nghìn đồng)" : "Price (thousand VND)"}
                      <input
                        type="number"
                        min="10"
                        max="500"
                    ...
                    <label className="inline-check">
                      ...
                      />
                      {ja ? "精進料理" : vi ? "Món chay" : "Vegetarian"}
                    </label>
                    ...
                    {editing && (
                      <button onClick={resetForm}>
                        {ja ? "キャンセル" : vi ? "Huỷ sửa" : "Cancel edit"}
                      </button>
                    )}
                  </div>
                  <div className="pool-list">
                    ...
                    {draft.custom.map((f) => (
                      <div className="pool-row" key={f.id}>
                        ...
                        <small>{priceLabel(f.price, language)}</small>
                        <button
                          aria-label={`${ja ? "削除" : vi ? "Xóa" : "Remove"} ${f.name}`}
                          onClick={() => {
                            updateDraft({
                              ...draft,
          ...
          <div className="pool-save">
            <small>
              ...
              {ja ? "品がケースに入っています" : vi ? "món trong hòm" : "dishes in case"} ·{" "}
              {dirty
                ? ja
                  ? "未保存"
                  : vi
          ...
          <div className="preferences-bottom">
            ...
            >
              {ja ? "クッキーを再読み込み" : vi ? "Tải lại cookie" : "Reload cookies"}
            </button>
            <button onClick={() => setConfirmDelete(!confirmDelete)}>
              {ja ? "保存リストをクリア" : vi ? "Xóa danh sách đã lưu" : "Clear saved dishes"}
            </button>
          </div>
          {confirmDelete && (
            <div className="delete-confirm">
              ...
              >
                {ja ? "削除を確定" : vi ? "Xác nhận xóa" : "Confirm deletion"}
              </button>
            </div>
          )}
